...
class InstalledCliTests(unittest.TestCase):
    ...
    def test_distribution_exposes_all_commands(self) -> None:
        entry_points = {
            ...
            for entry_point in metadata.distribution("datafileviewer").entry_points
            if entry_point.group == "console_scripts"
        }
        expected = "datafileviewer.cli:main"
        self.assertEqual(entry_points.get("datafileviewer"), expected)
        self.assertEqual(entry_points.get("dfv"), expected)
    ...
    def test_dfvt_implies_tui_mode(self) -> None:
        # dfvt is `datafileviewer --tui`; --terse is mutually exclusive with
        ...
        result = self.run_cli("dfvt", str(SAMPLE_FILE), "--terse")
    ...
    def test_parquet_file_without_pyarrow_reports_install_instructions(self) -> None:
        result = self.run_cli("datafileviewer", str(SAMPLE_PARQUET))
        self.assertEqual(result.returncode, 1)
        self.assertIn("pip install 'datafileviewer[parquet]'", result.stderr)
    ...
    def test_hdf5_file_without_h5py_reports_install_instructions(self) -> None:
        result = self.run_cli("datafileviewer", str(SAMPLE_HDF5))
        self.assertEqual(result.returncode, 1)
        self.assertIn("pip install 'datafileviewer[hdf5]'", result.stderr)
    ...
    def test_csv_file_without_pandas_reports_install_instructions(self) -> None:
        result = self.run_cli("datafileviewer", str(SAMPLE_CSV))
        self.assertEqual(result.returncode, 1)
        self.assertIn("reading .csv files needs: pandas", result.stderr)
        self.assertIn("pip install 'datafileviewer[pandas]'", result.stderr)
